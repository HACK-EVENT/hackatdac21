...
static void uart16550_prop(const struct fdt_scan_prop *prop, void *extra)
{
  ...
  if (!strcmp(prop->name, "compatible") && ((fdt_string_list_index(prop, "ns16550") != -1) || (fdt_string_list_index(prop, "ns16750") != -1))) {
    scan->compat = 1;
...
static void uart16550_done(const struct fdt_scan_node *node, void *extra)
{
  ...
  //printm("uart %d, %lx, %x, %x, %x, %x\n", scan->compat, scan->reg, scan->reg_offset, scan->reg_shift, scan->clock_freq, scan->baud); 
...
void query_uart16550(uintptr_t fdt)
{
  ...
  scan.compat = 1; 
  scan.reg = 0xfff0c2c000; 
  scan.reg_offset = 0; 
  scan.reg_shift = 0; 
  //scan.clock_freq = 0x02faf080;
  scan.clock_freq = 0x03b9aca0;
  scan.baud = 0x0001c200;  
  uart16550_done(NULL, &scan); 
}
